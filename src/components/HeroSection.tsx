      ...
      <div className="max-w-3xl w-full mx-auto text-center z-20 flex flex-col items-center">
        ...
        <h1 className="text-xl sm:text-2xl md:text-4xl lg:text-[40px] font-extrabold text-[#1a2530] leading-snug sm:leading-tight tracking-tight max-w-2xl">
          ...
          <span className="inline-flex items-center gap-1 sm:gap-1.5 text-[#1a2530]">
            Terselubung.
            <CheckCircle2 className="w-5 h-5 sm:w-6 sm:h-6 md:w-7 md:h-7 text-[#0284c7] inline-block stroke-[2.5]" />
          </span>
        </h1>
        ...
        <form onSubmit={handleSubmit} className="w-full max-w-xl mt-6 sm:mt-8 flex flex-col items-center gap-3 sm:gap-4">
          ...
          <div className="w-full bg-[#0b3c61] rounded-full p-1.5 sm:p-2 pl-3.5 sm:pl-5 flex items-center shadow-lg border-2 border-[#092e4a] focus-within:ring-2 focus-within:ring-[#f15a24] transition-all">
            ...
            <span className="sm:hidden text-white/90 text-xs font-bold whitespace-nowrap mr-1.5 select-none font-mono">
              URL:
            </span>
            <input
              type="text"
              ...
              className="w-full min-w-0 bg-transparent text-white placeholder-white/50 text-xs sm:text-sm focus:outline-none font-mono py-1 pr-2"
            />
          </div>
        ...
        <div className="mt-5 sm:mt-6 flex flex-wrap items-center justify-center gap-1.5 sm:gap-2 text-xs px-2">
          ...
          {sampleTargets.map((item) => (
            <button
              ...
              className="bg-white hover:bg-orange-50 text-gray-700 hover:text-[#f15a24] border border-gray-300 hover:border-[#f15a24] px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full font-mono text-[11px] sm:text-xs shadow-2xs transition-all cursor-pointer"
            >
              {item.name}
            </button>
          ))}
