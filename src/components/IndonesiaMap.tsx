...
export default function IndonesiaMap({ filter, apiData }: IndonesiaMapProps) {
  ...
  const [selectedRegionId, setSelectedRegionId] = useState<keyof typeof MAPPING>("jawa");
  ...
  const activeMapping = MAPPING[activeId];
  
  // Mengambil data spesifik pulau tersebut dari prop API
  // Jika API belum siap, gunakan fallback angka 100 (Aman)
  const activeStats = apiData && apiData[activeMapping.backendKey] 
    ? apiData[activeMapping.backendKey] 
    : { score: 100, infected: 0, total: 0, main_threat: "-", status: "AMAN / HIJAU" };

  const getFilterAdjustedScore = (baseScore: number) => {
    if (filter === "ac_id") return (baseScore > 5 ? baseScore - 5 : baseScore).toFixed(1);
    if (filter === "sch_id") return (baseScore < 95 ? baseScore + 4 : baseScore).toFixed(1);
  ...
  const isDanger = activeStats.score < 60;
  const isWarning = activeStats.score >= 60 && activeStats.score < 80;
    ...
    <div className="w-full flex flex-col items-center relative">
        ...
        <div className="bg-white/95 backdrop-blur-sm border-2 border-[#f15a24]/70 rounded-xl p-3 shadow-md text-left text-xs max-w-sm w-full transition-all">
          <div className="flex items-center justify-between pb-1.5 border-b border-gray-200">
            <span className="font-bold text-gray-800 flex items-center gap-1.5 truncate max-w-[200px]">
              {isDanger ? <ShieldAlert className="w-4 h-4 text-red-600 shrink-0" /> : isWarning ? <AlertTriangle className="w-4 h-4 text-amber-500 shrink-0" /> : <ShieldCheck className="w-4 h-4 text-emerald-600 shrink-0" />}
              {activeMapping.name}
            </span>
          ...
          <div className="grid grid-cols-2 gap-2 mt-2 font-mono text-[11px]">
            <div>
              ...
              <span className="font-bold text-gray-900 text-sm">
                {getFilterAdjustedScore(activeStats.score)}/100
              </span>
            </div>
            <div>
              <span className="text-gray-500 block text-[10px]">Domain Terdampak</span>
              <span className={`font-bold text-sm ${activeStats.infected > 0 ? "text-red-600" : "text-emerald-600"}`}>
                {activeStats.infected} / {activeStats.total}
              </span>
            </div>
          </div>
          ...
          <div className="mt-2 text-[10px] text-gray-600 bg-gray-50 p-1.5 rounded border border-gray-200">
            ...
            <p className={`font-mono mt-0.5 ${activeStats.infected > 0 ? "text-red-700" : "text-emerald-700"}`}>
              {activeStats.main_threat}
            </p>
          </div>
        </div>
      ...
      <div className="w-full relative border border-gray-300/70 rounded-2xl bg-white/60 p-2 md:p-6 shadow-inner overflow-hidden">
        <svg viewBox="0 0 950 420" className="w-full h-auto select-none" xmlns="http://www.w3.org/2000/svg">
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("sumatra")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("sumatra")}>
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("jawa")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("jawa")}>
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("bali_nusa")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("bali_nusa")}>
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("kalimantan")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("kalimantan")}>
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("sulawesi")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("sulawesi")}>
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("maluku")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("maluku")}>
          ...
          <g className="cursor-pointer transition-all duration-200" onMouseEnter={() => setHoveredRegionId("papua")} onMouseLeave={() => setHoveredRegionId(null)} onClick={() => setSelectedRegionId("papua")}>
        ...
        <div className="absolute bottom-2 right-4 text-[10px] text-gray-400 font-mono flex items-center gap-1">
          ...
          Peta ini terhubung langsung dengan Database Pemindaian Nasional.
        </div>
      </div>
    </div>
  );
