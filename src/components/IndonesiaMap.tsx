...
interface IndonesiaMapProps {
  filter: DomainFilter;
  apiData: any; // Menerima JSON utuh hasil dari Python Flask
}
...
export default function IndonesiaMap({ filter, apiData }: IndonesiaMapProps) {
  ...
  // Membaca data spesifik dari key "regions" buatan Python Anda
  const getRegionData = (backendKey: string) => {
    if (apiData?.regions && apiData.regions[backendKey]) {
      return apiData.regions[backendKey];
    }
    return { score: 100, infected: 0, total: 0, main_threat: "-", status: "AMAN / HIJAU" };
  };

  // Menghitung total kasus infected dari objek regions
  const getTotalInfected = () => {
    if (!apiData?.regions) return 0;
    return Object.values(apiData.regions).reduce((acc: number, curr: any) => acc + (curr.infected || 0), 0);
  };

  // Tentukan data yang ditampilkan (Data Nasional menggunakan agregat langsung dari backend!)
  const displayData = activeId 
    ? {
        name: MAPPING[activeId].name,
        ...getRegionData(MAPPING[activeId].backendKey)
      }
    : {
        name: "Indonesia (Nasional)",
        score: apiData?.national_index ?? 100,
        total: apiData?.total_scanned ?? 0,
        infected: getTotalInfected(),
        main_threat: (apiData?.threat_percentage ?? 0) > 0 ? "Ancaman Siber (Agregat Nasional)" : "Sistem Terpantau Bersih",
        status: (apiData?.national_index ?? 100) < 60 ? "BAHAYA / MERAH" : (apiData?.national_index ?? 100) < 80 ? "WASPADA / KUNING" : "AMAN / HIJAU"
      };
  ...
  return (
    <div className="w-full flex flex-col items-center relative">
        ...
        <div className="bg-white/95 backdrop-blur-sm border-2 border-[#f15a24]/70 rounded-xl p-3 shadow-md text-left text-xs max-w-sm w-full transition-all">
          ...
          <div className="grid grid-cols-2 gap-2 mt-2 font-mono text-[11px]">
            <div>
              ...
              <span className="font-bold text-gray-900 text-sm">{displayData.score}/100</span>
            </div>
            <div>
          ...
          <div className="mt-2 text-[10px] text-gray-600 bg-gray-50 p-1.5 rounded border border-gray-200">
            ...
            <p className={`font-mono mt-0.5 ${displayData.infected > 0 ? "text-red-700" : "text-emerald-700"}`}>{displayData.main_threat}</p>
          </div>
        </div>
      ...
      {/* SVG Map Container */}
      <div className="w-full relative border border-gray-300/70 rounded-2xl bg-white/60 p-2 md:p-6 shadow-inner overflow-hidden cursor-default" onClick={() => setSelectedRegionId(null)}>
        ...
        {/* Tulisan Petunjuk Reset Mode */}
        <div className="absolute bottom-2 right-4 text-[10px] text-gray-400 font-mono flex items-center gap-1 pointer-events-none">
